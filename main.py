...
def init_arduino():
    port = 'COM5'  # Убедитесь, что это правильный порт
    baudrate = 9600
    try:
...
def handle_joystick_input(ser):
    global last_input_time, last_input
    ...
    if ser and ser.in_waiting > 0:
        try:
            ...
            if line:
                ...
                # Игнорируем быстрые повторные нажатия
                if line == last_input and (current_time - last_input_time) < DEBOUNCE_DELAY:
                    return None
                
                last_input = line
                last_input_time = current_time
                print(f"Получена команда: {line}")
                return line
        except Exception as e:
...
def update_cursor_position(direction, current_mode, game_state):
    global current_cursor_pos, highlight_button, curr
    
    try:
        if current_mode == 'main_menu':
            print(123)
            if direction == '2':  # Кнопка Fight
                new_pos = (960, 500)
                highlight_button = 'fight'
            elif direction == '4':  # Кнопка View Stats
                new_pos = (960, 700)
                highlight_button = 'view'
        
        elif current_mode == 'view_stats':
            if direction == '2':  # Кнопка Back
                new_pos = (125, 365)
                highlight_button = 'back'
            elif direction == '4':  # Кнопка Next
                new_pos = (375, 365)
                highlight_button = 'next'
        elif current_mode == "fight" and game_state.status in ['select pokemon 1', 'select pokemon 2']:
            print(curr)
            if direction == '1' and curr >0:  # Кнопка Back
                curr -=1
            elif direction == '3' and curr < 2:  # Кнопка Next
                curr +=1
            if curr == 0:
                new_pos = (450, 500)
            elif curr == 1:
        ...
        elif current_mode == "fight" and game_state.status in ['player 1 turn', 'player 2 turn']:
            print(curr)
            if direction == '1' and curr >0:  # Кнопка Back
                curr -=1
            elif direction == '3' and curr < 2:  # Кнопка Next
        ...
        if current_mode == "fight" and game_state.status in ['player 1 move', 'player 2 move']:
            print(curr)
            if direction == '1' and curr >0:  # Кнопка Back
                curr -=1
            elif direction == '3' and curr < 2:  # Кнопка Next
        ...
        # Обновляем позицию и перемещаем курсор
        current_cursor_pos = new_pos
...
def main():
 ...
 # Обработка ввода с Arduino
        arduino_input = handle_joystick_input(ser) if ser else None  
        if arduino_input:
            if arduino_input in ['2', '4', '1', '3']:
                
                mouse_pos = update_cursor_position(arduino_input, current_mode, game_state)
                if mouse_pos:
                    # Создаем событие движения мыши
                    pygame.event.post(pygame.event.Event(
                        MOUSEMOTION,
                        {'pos': mouse_pos, 'rel': (0, 0), 'buttons': (0, 0, 0)}
                    ))
            elif arduino_input == '5':
                mouse_pos = simulate_mouse_click()
                if mouse_pos:
                    # Создаем события клика
                    pygame.event.post(pygame.event.Event(
                        MOUSEBUTTONDOWN,
